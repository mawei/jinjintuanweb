	...
	<div id="deal-default">
		...
		<div id="content">
			<div id="deal-intro" class="cf">
                <h1><?php if($team['close_time']==0){?><a class="deal-today-link" href="/team.php?id=<?php echo $team['id']; ?>">今日秒杀：</a><?php }?><?php echo $team['title']; ?></h1>
                <div class="main">
                    <div class="deal-buy">
					...
					<?php } else { ?>
                        <p class="deal-price"><strong><?php echo $currency; ?><?php echo moneyit($team['team_price']); ?></strong><span><a href="<?php echo $team['begin_time']<time()?'/team/buy.php?id='.$team['id'] : 'javascript: alert(\'秒杀还未开始...\')'; ?>"><img src="<?php echo $team['begin_time']<time()?'/static/css/i/button-deal-buy_seconds.gif' : '/static/css/i/button-deal-buy_seconds_wait.gif'; ?>" /></a></span></p>
					<?php }?>
                    </div>
					...
					<?php if($team['close_time']==0){?>
						...
						<div class="deal-box deal-status" id="deal-status">
							<?php if(($team['max_number'] - $team['now_number']) > 0){?>
								<p class="deal-buy-tip-top">还剩 <strong><?php echo ($team['max_number'] - $team['now_number']); ?></strong></p>
							<?php } else if((!$team['max_number'])) { ?>
								<p class="deal-buy-tip-top">秒杀进行中</p>
							<?php } else { ?>
								<p class="deal-buy-tip-top">秒杀已结束</p>
							<?php }?>
							...
							<p class="deal-buy-tip-btm">还差 <strong><?php echo $team['min_number']-$team['now_number']; ?></strong> 人到达最低秒杀人数</p>
						</div>
						<?php } else { ?>
						<div class="deal-box deal-status deal-status-open" id="deal-status">
							...
							<p class="deal-buy-on" style="line-height:200%;"><img src="/static/css/i/deal-buy-succ.gif"/> 秒杀成功！ <?php if($team['max_number']>$team['now_number']||$team['max_number']==0){?><br/>还可以继续秒杀<?php }?></p>
						</div>
						<?php }?>
					<?php } else { ?>
						<div class="deal-box deal-status deal-status-<?php echo $team['state']; ?>" id="deal-status"><div class="deal-buy-seconds"></div><p class="deal-buy-tip-total">共有 <strong><?php echo $team['now_number']; ?></strong> 人购买</p></div> 
					<?php }?>
				</div>
                <div class="side">
