...
<h3><span id="order-pay-dialog-close" class="close" onclick="return X.boxClose();">关闭</span>绑定手机号码,参团更流畅</h3>
	<p class="info" id="smssub-dialog-display-id">请您输入手机号及验证码<br/>获取验证码</p>
...
function smssub_submit() {
	...
	if(mobile == '') {
		...
		X.get(WEB_ROOT + "/ajax/sms.php?action=loginmobilebindcheck&mobile="+mobile+"&user_id=<?php echo $userid; ?>&verifycode="+verifycode+"&r="+ Math.random());
	}
}
