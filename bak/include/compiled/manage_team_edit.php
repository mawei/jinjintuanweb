            ...
            <div class="box-content">
				...
				<form id="-user-form" method="post" action="/manage/team/edit.php?id=<?php echo $team['id']; ?>" enctype="multipart/form-data" class="validator">
					...
					<div class="field">
                     ...
                     <div class="field" style="padding-left:46px;">
                        <p>项目标签：
                           <?php if($team['team_tags'] == 1){?>
                             <input type="checkbox" name="team_tags[]" value="1" checked="checked" />今日新单
                             <input type="checkbox" name="team_tags[]" value="2" />免预约
                          <?php } else if($team['team_tags'] == 2) { ?>  
                             <input type="checkbox" name="team_tags[]" value="1" />今日新单
                             <input type="checkbox" name="team_tags[]" value="2" checked="checked" />免预约
                          <?php } else if($team['team_tags'] == 12) { ?>  
                             <input type="checkbox" name="team_tags[]" value="1" checked="checked" />今日新单
                             <input type="checkbox" name="team_tags[]" value="2" checked="checked" />免预约
                          <?php } else { ?>   
                             <input type="checkbox" name="team_tags[]" value="1" />今日新单
					...
					<div class="field" style="padding-left:46px;">
						...
						<span style="width:80px;"><input type="checkbox" id="city_all" name="city_ids[]" value="0" <?php if($team['city_id'] == 0){?>checked<?php }?>/>全部</span>
					    </p>
						...
						<?php if(is_array($allcities)){foreach($allcities AS $index=>$one) { ?>
							<p>
								...
								<?php if(isset($one['id'])){?>
								   <?php 
								        $areas = get_areas($one['id']);
								   ; ?>
								  （<b>商圈：</b>
									<?php if($areas){?>
									   <?php if(is_array($areas)){foreach($areas AS $index=>$two) { ?>
									        <input type="checkbox" class="city_checkbox" name="area_ids[]" value="<?php echo $two['id']; ?>"
											<?php if(preg_match("/@".$two['id']."@/i", $team['area_ids'])){?>checked<?php }?> /><?php echo $two['name']; ?>
									   <?php }}?>
									 ）
	                                <?php }?> 
								<?php }?> 
					...
					<div class="field">
					<label>货到付款</label>
						<input type="checkbox" class="is_hdfk" name="is_hdfk" value="1" 

<?php if($team['is_hdfk']=='1'){?>checked<?php }?>/>&nbsp;是&nbsp;&nbsp;<span style="font-

size:12px;color:#989898;">本项目是否支持货到付款</span>						 
					</div>
                    <!--货到付款end-->
					<div class="field">
						<label>本单简介</label>
